...
def run(cfg):
    ...
    try:
        ...
        # 9-fix) garantir fórmulas N/O/P em itens com tipo+preço mas SEM P (A2: linha do "Painel
        #         elétrico" vinha como placeholder sem preço -> sem =N*I; senão some do total c/BDI)
        for r in range(15,210):
            g=retry(lambda r=r: ws.Cells(r,7).Value)
            if g in ('Mat','MO'):
                pf=retry(lambda r=r: ws.Cells(r,16).Formula)
                if not (isinstance(pf,str) and pf.startswith('=')):
                    retry(lambda r=r: setattr(ws.Cells(r,14),'Formula','=IF(K%d="","",K%d*(1+M%d))'%(r,r,r)))
                    retry(lambda r=r: setattr(ws.Cells(r,15),'Formula','=K%d*I%d'%(r,r)))
                    retry(lambda r=r: setattr(ws.Cells(r,16),'Formula','=N%d*I%d'%(r,r)))
