# -*- coding: utf-8 -*-
"""Config de obra — EEE SB-A4 — Altamira do Paraná/PR (Rua das Orquídeas - ASFALTO).
SB-A4 = MESMA EEE padronizada da SB-A3/A5. Verificado nos projetos da A4 (quadros do PEST):
concreto fck40 AGREGADO 10,88 m³ = 9,73+0,81+0,34 (poço+PV+abrigo do A3) EXATO; fôrma 62,76 m²
= 49,72+12,0+1,04 EXATO; aço 816,92 / 37,26 / 28,96 kg IDÊNTICOS; magro 0,52; profundidades
EEE 3,40 / PV 2,50 batem. Elétrico = abrigo + QCM (2 motores inversor 3CV, entrada 220V cat36
50A; sem QGBT/gerador/trafo); serviços 528/528/176/176/80 h idênticos. Motobomba em linha 3CV
1+1 (vazão/mca só no Memorial de Cálculo, ausente na pasta) -> item em linha continua None.
Difere da A3 só pelo SÍTIO: Rua das Orquídeas (asfalto), PV de chegada PV-047 (CT 564,579).
calcada=9,0: caixa 3,10x2,00 (confirmada no PEST forma + planta) + extensão 1,40 m do footprint
padronizado = 4,50x2,00 = 9,00 m² (mesmo da A3/A5). ASFALTO 9,0 m²/0,45 m³ = footprint repavi-
mentado (rua asfalto, implantação cita MEIO FIO/PAVIMENTO ASFALTICO/CALÇADA). Base: gabarito A2.
...
OUT_XLSX = _BASE + r"\NEXUS\ORC_EEE_SB-A4_Altamira.xlsx"
...
CP = {
 ...
 'm04':(126232.08,'Improv Equipamentos THS26.472 R00 (05/06/26) — SBL-220-SS-2P 6CV, 0,35 l/s / 20 mca (pacote: conj. em linha + painel + válvulas + dren.)'),
...
PRICE_UPD = {79:520.00, 81:126232.08, 83:6333.04, 89:647.00, 90:338.00}
QTY_UPD   = {}

# Bloco extra anexado antes do VALOR TOTAL (estação na RUA DAS ORQUÍDEAS = ASFALTO).
