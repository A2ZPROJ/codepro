# -*- coding: utf-8 -*-
"""Config de obra — EEE SB-A3 — Altamira do Paraná/PR (Rua Sebastião Brito Barbosa - ASFALTO).
SB-A3 = MESMA EEE padronizada da SB-A5: estrutura (10,88/0,52/62,76/816,92/37,26/28,96),
profundidades (EEE 3,40 / PV 2,50 / Fund -2,50 / N.R. 551,00), caixa 3,1x2,0, elétrica (PELE)
e materiais hidráulicos TODOS idênticos à A5 (verificado nos projetos). Difere só: motobomba
em linha 12,6 m³/h @ 30 mca (A5 era 9,97 mca) — pendente cotação. Base: gabarito A2.
calcada=9,0 CONFIRMADA pelo número real: caixa 3,10x2,00 = exata por coord. topográfica
(survey, 4 cantos no implantação PARQ-08-R1-02) + extensão 1,40 m (abrigo/base painel) =
cota EXATA medida no vetor do implantação (escala do detalhe ~1:75 = 37,8 pt/m, não 1:200).
Calçada = (3,10+1,40) x 2,00 = 4,50 x 2,00 = 9,00 m² (mesmo footprint padronizado da A5).
A PARQ-03 da A3 virou planta do barrilhete (não traz o retângulo do piso), por isso a fonte
exata é caixa-survey + cota-implantação. ASFALTO 9,0 m²/0,45 m³ = footprint da estação repavi-
mentado (rua asfalto), idêntico à A5. Recalque externo PENDENTE (Daniel) -> sem rede externa."""
import os
...
OUT_XLSX = _BASE + r"\NEXUS\ORC_EEE_SB-A3_Altamira.xlsx"
...
CP = {
 ...
 'm04':(126530.54,'Improv Equipamentos THS26.415 R01 (25/05/26) — SBL-230-SS-2P 7,5CV, 0,53 l/s / 30 mca (pacote: conj. em linha + painel + válvulas + dren.)'),
...
PRICE_UPD = {79:520.00, 81:126530.54, 83:6333.04, 89:647.00, 90:338.00}
QTY_UPD   = {}
